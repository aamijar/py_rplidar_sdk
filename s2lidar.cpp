...
int s2(int argc, const char * argv[]) {
    ...
    sl_u32         opt_channel_param_second = 1000000;
...
BOOST_PYTHON_MODULE(s2lidar)
{
    boost::python::def("init", s2Init);
    boost::python::def("get_scan", s2GetScan);
    boost::python::def("stop", s2Stop);
}
